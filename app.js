...
function calcBalances() {
  ...
  txns.filter(t => t.date >= OPENING_DATE).forEach(t => {
    ...
    if (t.type === 'Expense')  { if (bal[t.fromAccount] !== undefined) bal[t.fromAccount] -= amt; }
    else if (t.type === 'Income')   { if (bal[t.toAccount]   !== undefined) bal[t.toAccount]   += amt; }
...
function showConfirm(msg, onYes) {
  ...
  document.getElementById('confirmYes').onclick = () => { document.getElementById('confirmDialog').classList.remove('open'); onYes(); };
  document.getElementById('confirmNo').onclick  = () => document.getElementById('confirmDialog').classList.remove('open');
}
  ...
  grid.innerHTML += `
    <div style="background:rgba(248,113,113,0.07);border:1px solid rgba(248,113,113,0.3);border-radius:14px;padding:16px 18px;margin-bottom:10px;display:flex;justify-content:space-between;align-items:center">
      ...
      <div style="text-align:right">
        <div style="font-family:var(--mono);font-size:22px;font-weight:700;color:var(--expense)">-${fmtAmtDec(odBal)}</div>
...
function applyNL(p) {
  ...
  setTimeout(() => {
    ...
    if (p.type==='Transfer') { document.getElementById('txnFrom').value=p.fromAccount||''; document.getElementById('txnTo').value=p.toAccount||''; }
    if (p.category) {
...
function saveTxn() {
  ...
  let txn = { id:Date.now().toString(), date, amount, type:currentType, notes };
...
function renderReports() {
  ...
  document.getElementById('ledgerExpenses').innerHTML = EXP_CATS.map(cat => {
    ...
    const display = val===0 ? '<span style="color:var(--text3)">—</span>' : `<span style="color:var(--expense);font-family:var(--mono)">${fmtAmt(val)}</span>`;
  ...
  document.getElementById('ledgerIncome').innerHTML = INC_SUBS.map(sub => {
    ...
    const display = val===0 ? '<span style="color:var(--text3)">—</span>' : `<span style="color:var(--income);font-family:var(--mono)">${fmtAmt(val)}</span>`;
      ...
      <div style="margin-bottom:14px">
        ...
        <div style="padding-left:8px;border-left:2px solid var(--border)">
          ${data.entries.sort((a,b)=>new Date(b.date)-new Date(a.date)).map(t=>`
            <div style="display:flex;justify-content:space-between;padding:3px 0;border-bottom:1px solid rgba(42,42,58,0.4)">
...
function deleteReminder(id) {
  showConfirm('Delete reminder?',()=>{ reminders=reminders.filter(r=>r.id!==id); saveReminders(reminders); renderReminders(); showToast('Deleted','success'); });
}

function reminderToTxn(id) {
  const r=reminders.find(x=>x.id===id); if(!r) return;
  showPage('add');
  setTimeout(()=>{ document.getElementById('txnAmount').value=r.amount||''; document.getElementById('txnFrom').value=r.account; document.getElementById('txnNotes').value=r.title; showToast('Filled from reminder. Review & save.','success'); },150);
}
...
function checkReminders() {
  ...
  if(due.length){ banner.textContent=`🔔 Due today: ${due.map(r=>r.title).join(', ')}`; banner.style.display='block'; }
