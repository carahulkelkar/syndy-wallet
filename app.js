...
function renderTxnCards() {
  ...
  if (search) filtered = filtered.filter(t =>
    [t.category, t.subcategory, t.fromAccount, t.toAccount, t.notes, t.type].join(' ').toLowerCase().includes(search)
  );
  ...
  container.innerHTML = filtered.map(t => {
    ...
    const acc   = isExp ? t.fromAccount : isInc ? t.toAccount : `${t.fromAccount} → ${t.toAccount}`;
    return `
    <div class="txn-card" id="tc-${t.id}">
      <div class="txn-card-main" onclick="toggleTxnActions('${t.id}')">
        ...
        <div class="txn-card-body">
          ...
          <div class="txn-card-sub">${acc}${t.notes ? ' · ' + t.notes : ''}</div>
...
function toggleTxnActions(id) {
  ...
  document.querySelectorAll('.txn-card-chevron').forEach(el => { el.textContent = '›'; el.style.transform = ''; });
  if (!isOpen) {
